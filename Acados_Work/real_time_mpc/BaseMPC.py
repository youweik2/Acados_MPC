...
class GemCarOptimizer(object):
    ...
    def __init__(self, m_model, m_constraint, t_horizon, dt, grid):
        ...
        model = m_model
        self.pp = model.p
        self.grid = ca.reshape(self.pp, 50, 50)  # Reshape symbolic grid into 50x50 matrix
        self.rtg = grid
        ...
        self.nu = nu
        ny = nx + nu
        n_params = 50 * 50  # Total number of parameters
        ...
        safe_condition = [(0, 1.5), (1.5, 0), (-1.5, 0), (0, 0)
                          ,(2 * np.sin(np.pi / 4), 2 * np.cos(np.pi / 4))
                          ,(2 * np.sin(np.pi * 3 / 4), 2 * np.cos(np.pi * 3 / 4))
                        #   ,(2 * np.sin(np.pi * 5 / 4), 2 * np.cos(np.pi * 5 / 4))
                        #   ,(2 * np.sin(np.pi * 7 / 4), 1.5 * np.cos(np.pi * 7 / 4))
                          ]
        for safe in safe_condition:
            xp, yp = safe[0], safe[1]
            grid_value = 60 - self.get_gvalue(x_ + xp, y_ + yp, 10, -5, self.grid)
    ...
    def solve(self, x_real, y_real, theta_real, velocity_real):
        ...
        obs_1d = np.zeros((50 * 50,))
        for i in range(50):
            for j in range(50):
                obs_1d[i * 50 + j] = float(self.rtg[i][j])
