...
class GemCarModel(object):
    def __init__(self,):
        ...
        controls = ca.vertcat(v, omega) # fai: front angle
...
class GemCarOptimizer(object):
    ...
    def __init__(self, m_model, m_constraint, t_horizon, dt, obstacles, target):
        ...
        # Obstacle set here
        self.circle_obstacles_1 = {'x': 0, 'y': 20, 'r': 2.0}
        self.circle_obstacles_2 = {'x': 1, 'y': 25, 'r': 2.0}
        self.circle_obstacles_3 = {'x': -1, 'y': 30, 'r': 2.0}
        ...
        # Cost settings ***
        Q = np.diag([10.0, 5.0, 2.0])  # States
        R = np.array([[10.0, 0.0],[0.0, 1000.0]])            # Controls

        ocp.cost.cost_type = 'LINEAR_LS'
        ocp.cost.cost_type_e = 'LINEAR_LS'

        ocp.cost.W = scipy.linalg.block_diag(Q, R)
        ocp.cost.W_e = np.diag([10.0, 10.0, 1.0])
    ...
    # plot function
    def plot_results(self, start_x, start_y, theta_log, x_log, y_log, x_real_log, y_real_log, v_log, o_log):
    ...
    def main(self, x_init, y_init, theta_init, velocity_init):
        ...
        with tqdm(total=100, desc='cpu%', position=1) as cpubar, tqdm(total=100, desc='ram%', position=0) as rambar:
            for i in tqdm(range(self.Epi)):
                ...
                try:
                    x_0, y_0, theta, vel, o_0 = self.solve(x_real, y_real, theta_real, vel_real)
                    ...
                    if (x_0 - self.target_x) ** 2 + (y_0 - self.target_y) ** 2 < 1:
                        ...
                        if self.plot_figures == True:
                            self.plot_results(x_init, y_init, theta_log, x_log, y_log, x_real_log, y_real_log, v_log, o_log)
...
if __name__ == '__main__':
    ...
    obstacles = np.array([
    [0.0, 20, 2],        #x, y, r 20 25 30
    [-1.0, 25, 2],
    [1.0, 30, 2]
    ])

    start_x, start_y, theta, vel = -0.25, -0.3, np.pi/2, 0.0007
    terminal = np.array([0.0, 50.0, np.pi/2])
