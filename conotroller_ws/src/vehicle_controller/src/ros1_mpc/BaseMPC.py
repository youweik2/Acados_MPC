...
class GemCarOptimizer(object):
    ...
    def __init__(self, m_model, m_constraint, t_horizon, dt, obstacles):
        ...
        R = np.array([[5.0, 5.0],[5.0, 50.0]])            # Controls
    ...
    def plot_results(self, start_x, start_y, theta_log, a_log, x_log, y_log, x_real_log, y_real_log, o_log):
        ...
        plt.plot(v, o_log, 'r-', label='desired omega')
        plt.xlabel('time')
        plt.ylabel('value')
        plt.legend()
        plt.grid(True)
...
if __name__ == '__main__':
    ...
    obstacles = np.array([
    [2.0, 10, 1],        #x, y, r 20 25 30
    # [-1.0, 25, 4],
    # [1.0, 30, 4]
    ])

    start_x, start_y, theta, vel = -2.99, 2.75, 0.0/180.0*np.pi, 0.0
